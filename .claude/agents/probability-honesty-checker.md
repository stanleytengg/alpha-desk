...
**Base rate 欄位強制格式：`N/8 beat, +X.X% avg`（從 `briefing-out/cache/earnings-history.json` 讀，主 skill 必須在 prompt 內帶入）。寫成「應該會 beat」「歷史不錯」等質性語言一律 INVALID。**
...
cache 缺資料的 ticker → 填 `(unavailable)` 並在 Step 2 形狀反推時降級信心（拉寬區間）。
...
### 1h. Thesis 健康度
- Fundamental thesis intact: [X/Y]
- Thesis 破裂或惡化中: [list]

### 1i. Macro state（從 `briefing-out/cache/macro-snapshot.json` 讀，主 skill 必須帶入）
